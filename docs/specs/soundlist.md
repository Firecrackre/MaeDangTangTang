...
* 옵션 결과음은 옵션이 공개될 때마다 한 번만 재생한다.
* 옵션 결과음은 옵션 한 줄이 모두 공개된 후 재생한다.
